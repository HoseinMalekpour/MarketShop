...
namespace App.Infrastructures.Data.Repositories.DataAccess.Repositories
{
    public class ProductRipository:IProductRipository
    {
        ...
        public ProductRipository(IMapper mapper, MarketDbContext context)
        {
            _mapper = mapper;
            _context = context;
        }

        public async Task Active(ProductDto product, CancellationToken cancellationToken)
        {
        ...
        public async Task DiActive(ProductDto product, CancellationToken cancellationToken)
        {
            var order = await _context.Products
             .Where(x => x.Id == product.Id)
             .FirstOrDefaultAsync();
